...
while (iter<max_it) && (ntol > tol) && (sigma2 > 10*eps)
    ...
    if ~isempty(idx)
       sprintf('X isnan or isinf')
       X
    end
    idx_2 = find( isnan(T) | isinf(T) );
    if ~isempty(idx_2)
        sprintf('T isnan or isinf' )
        T
    end
    
    idx_x1 = find( ~isinf(X(:,1)) & ~isnan(X(:,1)) );
    idx_x2 = find( ~isinf(X(:,2)) & ~isnan(X(:,2)) );
    idx_x3 = find( ~isinf(X(:,3)) & ~isnan(X(:,3)) );
    
    idx_x12 = intersect( idx_x1,idx_x2 );
    idx_x = intersect( idx_x12, idx_x3 );
    
    X = X(idx_x,:);
    
    idx_t1 = find( ~isinf(T(:,1)) & ~isnan(T(:,1)) );
    idx_t2 = find( ~isinf(T(:,2)) & ~isnan(T(:,2)) );
    idx_t3 = find( ~isinf(T(:,3)) & ~isnan(T(:,3)) );
    
    idx_t12 = intersect( idx_t1,idx_t2 );
    idx_t = intersect( idx_t12, idx_t3 );    
