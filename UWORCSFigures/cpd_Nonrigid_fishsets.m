...
X2=[X(1:end-10,:); 0.3*randn(10,2)];
Y2=[Y(15:end-15,:); 0.3*randn(10,2)];
...
[Transform2,C2]=cpd_register(X2,Y2,opt);

figure,cpd_plot_iter(X, Y); title('Before');
figure,cpd_plot_iter(X2,Y2); title('Before');
figure,cpd_plot_iter(X, Transform.Y, C);  title('After registering Y to X. And Correspondences');
figure,cpd_plot_iter(X2, Transform2.Y, C2);  title('After registering Y to X. And Correspondences');