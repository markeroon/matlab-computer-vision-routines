function [JL,pml1,JR,pmr1] = rectifyCalibratedImagePair( IL,pml,IR,pmr )
...
for c = 1:3
    ...
    % Warp LEFT
    [JL(:,:,c),bbL,alphaL] = imwarp(IL(:,:,c), TL, 'bilinear', 'same');

    % Warp RIGHT
    [JR(:,:,c),bbR,alphaR] = imwarp(IR(:,:,c), TR, 'bilinear', 'same');
