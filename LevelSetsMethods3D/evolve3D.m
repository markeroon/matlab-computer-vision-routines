function [phi] = evolve3D(phi, dx, dy, dz, alpha, iterations, accuracy, is_signed_distance, normal_evolve, Vn, vector_evolve, u, v, w, kappa_evolve, b)
%
% function [phi] = evolve2D(phi, dx, dy, dz, iterations, accuracy, ...
%   is_signed_distance, normal_evolve, Vn, vector_evolve, u, v, w, kappa_evolve, b)
%
% Calculates evolution for a 3D curve (4D level set function) phi.
...
switch(evolution_type)
	case 'normal_vector_kappa'
		...
		[u_ext, v_ext,w_ext] = feval(init_vector, u, v, w);
		[dx2, dy2, dz2] = init_kappa(dx, dy, dz);
		it=0;
		t = 0;
		while(it < iterations)
			[delta_normal_vector, H1_abs, H2_abs] = feval(evolve_normal_vector, ...
				phi, dx, dy, dz, Vn_ext, u_ext, v_ext, w_ext);
			delta_kappa = evolve_kappa(phi, dx, dy, dz, b, dx2, dy2, dz2);
	...
	case 'normal_vector'
		...
		t = 0;
		while(it < iterations)
			[delta_normal_vector, H1_abs, H2_abs, H3_abs] = feval(evolve_normal_vector, ...
				phi, dx, dy, dz, Vn_ext, u_ext, v_ext, w_ext);
			dt = get_dt_normal_vector(alpha, dx, dy, dz, H1_abs, H2_abs, H3_abs);
	...
	case 'vector_kappa'
		[u_ext, v_ext,w_ext] = feval(init_vector, u, v, w);
		[dx2, dy2, dz2] = init_kappa(dx, dy, dz);
		dt = get_dt_vector_kappa(alpha, dx, dy, dz, u, v, w, b, dx2, dy2, dz2);
		it=0;
		t = 0;
		while(it < iterations)
			delta_vector = feval(evolve_vector, phi, dx, dy, dz, u_ext, v_ext, w_ext);
			delta_kappa = evolve_kappa(phi, dx, dy, dz, b, dx2, dy2, dz2);
			phi = phi - dt*(delta_kappa + delta_vector); % THIS WAS CHANGED ON DEC20!!
			it = it+1
			t = t+dt;
		end
	case 'vector'
		[u_ext, v_ext, w_ext] = feval(init_vector, u, v, w);
		dt = get_dt_vector(alpha, dx, dy, dz, u, v, w);
        %h = min( [dx dy dz] );
        %dt = h*h;
        it=0;
		t = 0;
		while(it < iterations)
			delta_vector = feval(evolve_vector, phi, dx, dy, dz, u_ext, v_ext, w_ext);
			phi = phi - dt*delta_vector; % CHANGED ON DEC 20!!!
			it = it+1
			t = t+dt;
		end
	case 'kappa'
		[dx2, dy2, dz2] = init_kappa(dx, dy, dz);
		dt = get_dt_kappa(alpha, dx, dy, dz, b, dx2, dy2, dz2);
		it=0;
		t = 0;
		while(it < iterations)
			delta_kappa = evolve_kappa(phi, dx, dy, dz, b, dx2, dy2, dz2);            
