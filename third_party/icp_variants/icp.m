...
%       q and specifies which points of q lay on the boundary.
%
% EdgeRejection
%       {false} | true
...
%       v. 7.3 or higher.
%       normalShooting finds the point closest to the normal line surface
%       intersection using kdTrees. It can only be used with point to point
%       minimization without EdgeRejection.
%
% Minimize
...
% NormalsData
%       {[]} | m x 3 matrix
%       A matrix of normals for the m points in p might be provided.
%       Normals of p are used for normalShooting matching.
%       Else normals will be found through a PCA of the 4 nearest
%       neighbors.
%
% ReturnAll
...
validMatching = {'bruteForce','normalShooting','Delaunay','GLtree','kDtree'};
...
% If Matching == 'GLtree', a GL tree should be built
...
if strcmp(arg.Matching, 'kDtree') || strcmp(arg.Matching, 'normalShooting')
    ...
    if arg.Extrapolation
        ...
        if k>2 && theta(k+1) < 10 && theta(k) < 10
            ...
            if dv ~= 0
                ...
                [nnn, mindist] = match_kDtree(q,pt,kdOBJ);
...
function [match mindist] = match_kDtree(nnnnn, p, kdOBJ)
...
[U,nnnn,V] = svd(N); % singular value decomposition
