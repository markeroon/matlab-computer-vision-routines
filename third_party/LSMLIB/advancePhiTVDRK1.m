%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% advancePhiTVDRK1 advances the level set function in time using a 
% first-order Runge-Kutta (i.e. Forward Euler) step.
%
% Usage: phi_next = advancePhiTVDRK1(phi_cur, velocity, ...
%                                    ghostcell_width, ...
%                                    dX, dt, ...
%                                    cfl_number, ...
...
function phi_next = advancePhiTVDRK1(phi_cur, velocity, ...
                                     ghostcell_width, ...
                                     dX, dt, ...
                                     cfl_number, ...
                                     spatial_derivative_order)
...
% parameter checks
if (nargin < 5)
  error('MATLAB:missingArgs','advancePhiTVDRK1:missing arguments');
end
...
if (nargin < 6)
  ...
  if (cfl_number <= 0)
    error('advancePhiTVDRK1:CFL number must be positive');
  end
end
...
if (nargin < 7)
  ...
  if ( (spatial_derivative_order ~= 1) & (spatial_derivative_order ~= 2) ...
    ...
    error('advancePhiTVDRK1:Invalid spatial derivative order...only 1, 2, 3, and 5 are supported');
  end
end
...
if (num_dims < 1 | num_dims > 3)
  error('advancePhiTVDRK1:Invalid dimension...only 1D, 2D, and 3D problems supported');
end

% check that the phi_cur data array is larger than the velocity data array
if ( ~isempty(find(size(phi_cur)<size(velocity{1}))) )
  error('advancePhiTVDRK1:velocity data array is smaller than phi_cur data array in at least one dimension.');
end
