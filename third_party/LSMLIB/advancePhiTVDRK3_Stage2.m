%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% advancePhiTVDRK3_Stage2 advances the level set function through the second
% stage of a third-order TVD Runge-Kutta step.
%
% Usage: phi_stage2 = advancePhiTVDRK3_Stage2(phi_stage1, phi_cur, ...
%                                             velocity, ...
%                                             ghostcell_width, ...
%                                             dX, dt, ...
%                                             cfl_number, ...
...
function phi_stage2 = advancePhiTVDRK3_Stage2(phi_stage1, phi_cur, ...
                                              velocity, ...
                                              ghostcell_width, ...
                                              dX, dt, ...
                                              cfl_number, ...
...
% parameter checks
if (nargin < 5)
  error('MATLAB:missingArgs','advancePhiTVDRK3_Stage2:missing arguments');
end
...
if (nargin < 6)
  ...
  if (cfl_number <= 0)
    error('advancePhiTVDRK3_Stage2:CFL number must be positive');
  end
end
...
if (nargin < 7)
  ...
  if ( (spatial_derivative_order ~= 1) & (spatial_derivative_order ~= 2) ...
    ...
    error('advancePhiTVDRK3_Stage2:Invalid spatial derivative order...only 1, 2, 3, and 5 are supported');
  end
end
...
if (num_dims < 1 | num_dims > 3)
  error('advancePhiTVDRK3_Stage2:Invalid dimension...only 1D, 2D, and 3D problems supported');
end

% check that the phi_cur data array is larger than the velocity data array
if ( ~isempty(find(size(phi_cur)<size(velocity{1}))) )
  error('advancePhiTVDRK3_Stage2:velocity data array is smaller than phi_cur data array in at least one dimension.');
end
...
% take time step
phi_stage2 = 0.75*phi_cur + 0.25*(phi_stage1 + dt*lse_rhs);
